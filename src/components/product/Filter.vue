						...
						<div class="filter__items">
							...
							<div class="filter__item" id="filter__genre">
								...
								<vue-custom-scrollbar :key="'drop-genre'" tagname="ul" :settings="settings" class="dropmenu-genre filter__item-menu dropdown-menu scrollbar-dropdown" aria-labelledby="filter-genre">
									...
									<li v-for="genre in genres" :data-id="genre.id" :class="{active: genre.id === storage.get('genre')}" :key="`genre`+genre.id" @click="selectItemDropDown(genre, 'genre')">{{genre.name}}</li>
...
export default {
  ...
  methods: {
    ...
    verifyQueryParams(){
        const company = this.$route.query.compaby;
    }    
  },
    ...
    genres(){    
        if( this.paramGenre ){
            ...
            this.genres.forEach(function(item){            
                if(item.id === parseInt(scope.paramGenre) ){
                    scope.storage.set('genre_name', item.name);
                    return;
                }
            });
            this.filtering();       
